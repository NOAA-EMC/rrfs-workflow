...
This is the ex-script for the task that runs an analysis with RRFS for
the specified cycle.
========================================================================"
#
#-----------------------------------------------------------------------
...
if [ "${DO_RADDA}" = "TRUE" ]; then
  ...
  if [ -r ${DATA} ]; then
     ...
     if [ ${numfile_cnv} -gt 0 ]; then
        ...
        cpreq -p ./rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_cnvstat_nc  ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_cnvstat
     fi
     if [ ${numfile_rad} -gt 0 ]; then
        tar -cvzf rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat_nc `cat listrad`
        cpreq -p ./rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat_nc  ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat
     fi
     if [ ${numfile_rad_bin} -gt 0 ]; then
        tar -cvzf rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat `cat listrad_bin`
        cpreq -p ./rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat  ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat
     fi

     # For EnVar DA  
     if [ -r ./satbias_out ]; then
       cpreq -p ./satbias_out ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_satbias
       cpreq -p ./satbias_out ${COMOUT}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_satbias
     fi
     if [ -r ./satbias_pc.out ]; then
       cpreq -p ./satbias_pc.out ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_satbias_pc
...
if [ "${DO_RADMON}" = "TRUE" ]; then 
   if [ ! -f ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat ]; then
     ...
     export biascr=${biascr:-${SATBIAS_DIR_NEW}/${RADMON_SUFFIX}.${CYCLE_TYPE}.${rgnHH}_satbias}
     export radstat=${radstat:-${SATBIAS_DIR_NEW}/${RADMON_SUFFIX}.${CYCLE_TYPE}.${rgnHH}_radstat}
