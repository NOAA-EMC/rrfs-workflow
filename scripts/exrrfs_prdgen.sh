...
# Add 2dfld tasks to the parallel script
  ...
  if [ ${DO_ENSFCST} = "FALSE" ]; then
    ...
    if [[ $SENDCOM = 'YES' ]]; then
       export gridspecs="rot-ll:247.000000:-35.000000:0.000000 299.000000:1127:0.108300 -36.9303000:683:0.108300"
      wgrib2 ${COMOUT}/${prslev} -new_grid_vectors "UGRD:VGRD:USTM:VSTM" -submsg_uv inputs.gribprslev13km.uv
      wgrib2 inputs.gribprslev13km.uv -set_bitmap 1 -set_grib_type c3 \
        -new_grid_winds grid -new_grid_vectors "UGRD:VGRD:USTM:VSTM" \
        -new_grid_interpolation neighbor \
        -if ":(WEASD|APCP|NCPCP|ACPCP|SNOD):" -new_grid_interpolation budget -fi \
        -new_grid ${gridspecs} ${COMOUT}/${prslev_na_13km}
      wgrib2 ${COMOUT}/${prslev_na_13km} -s > ${COMOUT}/${prslev_na_13km}.idx

      wgrib2 ${COMOUT}/${fld2d} -new_grid_vectors "UGRD:VGRD:USTM:VSTM" -submsg_uv inputs.grib2dfld13km.uv
      wgrib2 inputs.grib2dfld13km.uv -set_bitmap 1 -set_grib_type c3 \
        -new_grid_winds grid -new_grid_vectors "UGRD:VGRD:USTM:VSTM" \
        -new_grid_interpolation neighbor \
        -if ":(WEASD|APCP|NCPCP|ACPCP|SNOD):" -new_grid_interpolation budget -fi \
        -new_grid ${gridspecs} ${COMOUT}/${fld2d_na_13km}
