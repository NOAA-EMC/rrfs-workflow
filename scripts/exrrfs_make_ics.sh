...
if [[ $DO_ENS_BLENDING = "FALSE" ]]; then
  ...
  cpreq -p out.atm.tile${TILE_RGNL}.nc ${shared_output_data}/gfs_data.tile${TILE_RGNL}.halo${NH0}.nc

  #### mv out.sfc.tile${TILE_RGNL}.nc \
  ####       ${DATA}/sfc_data.tile${TILE_RGNL}.halo${NH0}.nc
  cpreq -p out.sfc.tile${TILE_RGNL}.nc ${shared_output_data}/sfc_data.tile${TILE_RGNL}.halo${NH0}.nc

  #### mv gfs_ctrl.nc ${DATA}
  cpreq -p gfs_ctrl.nc ${shared_output_data}

  #### mv gfs.bndy.nc ${DATA}/gfs_bndy.tile${TILE_RGNL}.000.nc
  cpreq -p gfs.bndy.nc ${shared_output_data}/gfs_bndy.tile${TILE_RGNL}.000.nc
