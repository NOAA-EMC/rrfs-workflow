...
if [ -r "${shared_forecast_restart_data}/${restart_prefix}.coupler.res" ]; then
  ...
  #-----------------------------------------------------------------------
  #
  # Ursa (strip when merging to the nco branch): the forecast writes these files one at a time, so
  # coupler.res above can be there while the rest are still being written, and the copy below fails
  # on the first missing one. Retro reruns hit the same gap. Wait for the whole set first.
