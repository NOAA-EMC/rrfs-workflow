...
if [[ "${SENDCOM}" = "YES" ]]; then
 ...
 if [[ "${SENDDBN}" = "YES" ]]; then
 if [ ${WGF} = "det" ]; then
   $DBNROOT/bin/dbn_alert MODEL RRFS_DET_BUFR $job ${COMOUT}/rrfs.t${cyc}z.class1.bufr
elif [ ${WGF} = "ensf" ]; then
...
if [ -d ${COMOUT}/bufr.${cyc} ]; then
  cd $COMOUT
  [ -d bufr.${cyc} ]&& rm -r bufr.${cyc}
  cd $DATA
fi

[ -f stnmlist_input ]&& rm stnmlist_input
...
if [[ "${SENDCOM}" = "YES" ]]; then
 ...
 if [[ "${SENDDBN}" = "YES" ]]; then
  if [ ${WGF} = "det" ]; then
    ...
    $DBNROOT/bin/dbn_alert MODEL RRFS_DET_SOUNDING $job ${COMOUT}/rrfs.t${cyc}z.bufrsnd.tar.gz
  elif [  ${WGF} = "ensf" ]; then
    ...
    $DBNROOT/bin/dbn_alert MODEL RRFS_ENS_SOUNDING $job ${COMOUT}/rrfs.t${cyc}z.bufrsnd.tar.gz
  fi
 fi
fi
