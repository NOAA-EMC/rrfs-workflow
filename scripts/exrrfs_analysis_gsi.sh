...
#    else
    # For EnVar
    #
    SATBIAS_DIR_NEW=${SATBIAS_DIR}/satbias.${YYYYMMDD}
    if [ ! -e $SATBIAS_DIR_NEW ]; then
	   mkdir -p $SATBIAS_DIR_NEW
    fi 

    if [ -r ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${SAT_TIME}_satbias ]; then
      echo " using satellite bias files from ${SATBIAS_DIR_NEW} ${spinup_or_prod_rrfs}.${SAT_TIME}"
      cpreq -p ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${SAT_TIME}_satbias ./satbias_in
      cpreq -p ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${SAT_TIME}_satbias_pc ./satbias_pc
      if [ -r ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${SAT_TIME}_radstat ]; then
         cpreq -p ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${SAT_TIME}_radstat ./radstat.rrfs
      fi
  ...
  ## if satbias files (go back to previous 10 days) are not available from ${SATBIAS_DIR_NEW}, use satbias files from the ${FIX_GSI} 
...
if [ "${DO_GSIDIAG_OFFLINE}" = "FALSE" ]; then
  ...
  if [ "${DO_RADDA}" = "TRUE" ]; then
    ...
    if [ ${numfile_cnv} -gt 0 ]; then
      ...
      cpreq ./rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_cnvstat_nc  ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_cnvstat
    fi
    if [ ${numfile_rad} -gt 0 ]; then
      tar -cvzf rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat_nc `cat listrad`
      cpreq ./rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat_nc  ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat
    fi
    if [ ${numfile_rad_bin} -gt 0 ]; then
      tar -cvzf rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat `cat listrad_bin`
      cpreq ./rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat  ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_radstat
    fi

    # For EnVar DA  
    cpreq ./satbias_out ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_satbias
    cpreq ./satbias_pc.out ${SATBIAS_DIR_NEW}/rrfs.${spinup_or_prod_rrfs}.${YYYYMMDDHH}_satbias_pc
