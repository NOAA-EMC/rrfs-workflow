...
if [ "${DO_NON_DA_RUN}" = "TRUE" ]; then
  target="${DATAROOT}/${RUN}_${PDY}${cyc}/${WGF}/ics/gfs_data.tile${TILE_RGNL}.halo${NH0}.nc"
else
  if [ ${BKTYPE} -eq 1 ]; then
...
if [ "${DO_NON_DA_RUN}" = "TRUE" ]; then
  target="${DATAROOT}/${RUN}_${PDY}${cyc}/${WGF}/ics/sfc_data.tile${TILE_RGNL}.halo${NH0}.nc"
  symlink="sfc_data.nc"
  ln -sf ${relative_or_null} $target $symlink

  target="${DATAROOT}/${RUN}_${PDY}${cyc}/${WGF}/ics/gfs_ctrl.nc"
  symlink="gfs_ctrl.nc"
  ln -sf ${relative_or_null} $target $symlink

  target="${DATAROOT}/${RUN}_${PDY}${cyc}/${WGF}/ics/gfs_bndy.tile${TILE_RGNL}.000.nc"
  symlink="gfs_bndy.tile${TILE_RGNL}.000.nc"
  ln -sf ${relative_or_null} $target $symlink

  for fhr in $(seq -f "%03g" ${LBC_SPEC_INTVL_HRS} ${LBC_SPEC_INTVL_HRS} ${FCST_LEN_HRS}); do
    target="$(compath.py -o ${NET}/${rrfs_ver}/${RUN}.${PDY}/${cyc})/lbcs/gfs_bndy.tile${TILE_RGNL}.${fhr}.nc"
