...
while [[ $n -le 12 ]]; do
  ...
  HHlbc=${CDATElbc:8:2}
  checkfile=${COMINrrfs}/rrfs.${YYYYMMDDlbc}/${HHlbc}${MEMDIR}/lbc/${last_bdyfile}
  if [ -r "${checkfile}" ];then
...
if [ -r "${checkfile}" ]; then
  for fhr in  ${fhr_all}; do
    ...
    ${cpreq} ${COMINrrfs}/rrfs.${YYYYMMDDlbc}/${HHlbc}${MEMDIR}/lbc/lbc.${string_time}.nc ${UMBRELLA_DATA}${MEMDIR}/prep_lbc/.
  done
else
