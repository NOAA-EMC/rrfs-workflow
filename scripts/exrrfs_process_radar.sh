...
for bigmin in ${RADARREFL_TIMELEVEL[@]}; do
  ...
  if [ -s filelist_mrms ]; then
     ...
     print_info_msg "$VERBOSE" "NSSL grib2 file levels = $numgrib2"
	  ...
	  echo "WARNING: Not enough radar reflectivity files in ${NSSL} available for loop ${bigmin}.  Will continue without it (is suboptimal but not a huge concern if does not persist)"
     continue
  fi
  #
  #-----------------------------------------------------------------------
