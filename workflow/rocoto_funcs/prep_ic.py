...
def prep_ic(xmlFile, expdir, do_ensemble=False, spinup_mode=0):
    ...
    datadep_prod = f'''\n      <datadep age="00:00:05"><cyclestr offset="-{cyc_interval}:00:00">&COMROOT;/&NET;/&rrfs_ver;/&RUN;.@Y@m@d/@H/fcst/&WGF;/save_for_next_f{cyc_interval:0>2}.done</cyclestr></datadep>'''
    ...
    if sfc_update_cycs != '99':
        ...
        for i in range(1, int(sfc_update_look_back_hrs) + 1, 1):
            datadep_sfc = datadep_sfc + f'''\n        <datadep age="00:00:05"><cyclestr offset="-{i}:00:00">&COMROOT;/&NET;/&rrfs_ver;/&RUN;.@Y@m@d/@H/fcst/&WGF;/save_for_next.f{i:0>2}.done</cyclestr></datadep>'''
        sfc_dep = f'''
    <and>
      <or>
{sfc_streqs}
