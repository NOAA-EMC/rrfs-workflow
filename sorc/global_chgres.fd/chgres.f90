...
! -- Henry Juang's approach
        ...
        IF (REGIONAL < 2) THEN
          IF (ISGRBSRC <= 1) THEN 
			  CALL WRITE_FV3_ATMS_NETCDF(GFSDATAO%ZS,GFSDATAO%PS,GFSDATAO%T,GFSDATAO%W,  &
					 ...
					 LONB,LATB,LEVSO,NTRACO,NVCOORDO,NTILES,HALO,INPTYP,MODELNAME)
		  ELSE
		      CALL WRITE_FV3_ATMS_NETCDF_GRBSRC(GFSDATAO%ZS,GFSDATAO%PS,GFSDATAO%T,GFSDATAO%W,  &
					 GFSDATAO%U,GFSDATAO%V,GFSDATAO%Q,VCOORDO,         &
					 LONB,LATB,LEVSO,NTRACO,NVCOORDO,NTILES,HALO,INPTYP,MODELNAME,KGDS_INPUT)
		  ENDIF
        ENDIF
        ...
        IF (REGIONAL >=1) THEN
		  ...
		  ELSE
		      CALL WRITE_FV3_ATMS_BNDY_NETCDF_GRBSRC(GFSDATAO%ZS,GFSDATAO%PS,GFSDATAO%T,   &
					   GFSDATAO%W,GFSDATAO%U,GFSDATAO%V,GFSDATAO%Q,VCOORDO,         &
					   LONB,LATB,LEVSO,NTRACO,NVCOORDO,HALO,INPTYP,MODELNAME,KGDS_INPUT)
		  ENDIF
        ENDIF
      ...
      ELSEIF(INPTYP==1) THEN
		...
		print *, 'CALL READ_GFS_SFC_HEADER_NEMSIO'
          CALL READ_GFS_SFC_HEADER_NEMSIO (IMI,JMI,IVSI,LSOILI,  &
                              FCSTHOUR,IDATE4O,KGDS_INPUT)
        ELSEIF (ISGRBSRC .GE. 2) THEN
        PRINT *, 'CALL READ_NAM_SFC_HEADER_NEMSIO'
          CALL READ_NAM_SFC_HEADER_NEMSIO (IMI,JMI,IVSI,LSOILI,  &
                              FCSTHOUR,IDATE4O,KGDS_INPUT)

        ENDIF
      ENDIF
      
      print *, 'in chgres after read header ', KGDS_INPUT
...
!  CODE WILL NOT INTERPOLATE THEM.
      ...
      IF (IVSI >= 200501 .AND. ISGRBSRC .LE. 0) THEN
...
!  BY THE NEW SURFACE CHGRES LOGIC.   
      ...
      IF (ISGRBSRC > 0) THEN
		  ...
		  ELSE
		  	  CALL EC2ANY(0,1,T2MI,IMI,JMI,T2MO,IMO,JMO,GEOLON,GEOLAT,KGDS_INPUT)
			  CALL EC2ANY(0,1,Q2MI,IMI,JMI,Q2MO,IMO,JMO,GEOLON,GEOLAT,KGDS_INPUT)
			  CALL EC2ANY(2,1,SRFLAGI,IMI,JMI,SRFLAGO,IMO,JMO,GEOLON,GEOLAT,KGDS_INPUT)
			  CALL EC2ANY(0,1,F10MI,IMI,JMI,F10MO,IMO,JMO,GEOLON,GEOLAT,KGDS_INPUT)
		  ENDIF
      ELSE
          CALL GL2ANY(0,1,F10MI,IMI,JMI,F10MO,IMO,JMO,GEOLON,GEOLAT)
          CALL GL2ANY(0,1,T2MI,IMI,JMI,T2MO,IMO,JMO,GEOLON,GEOLAT)
