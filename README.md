...
```
where `[machine]` is `wcoss2`, `hera`, `jet`, `orion`, or `hercules`.  The `--nogtg` flag avoids compilation of GTG and IFI items that only select users can compile.
