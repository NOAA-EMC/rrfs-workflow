...
OPTIONS
  ...
  --install-dir=INSTALL_DIR  
      installation  directory name (\${HOME_DIR} by default)
  --build-dir=BUILD_DIR
      main build directory, absolute path (\${HOME_DIR}/build/ by default)
  --bin-dir=BIN_DIR
      binary directory name ("exec" by default); full path is \${INSTALL_DIR}/\${BIN_DIR})
  --sub-modules
      remove sub-module directories. They will need to be checked out again by sourcing "\${HOME_DIR}/manage_externals/checkout_externals" before attempting subsequent builds
  -v, --verbose
...
BUILD_DIR=${BUILD_DIR:-"${HOME_DIR}/build"}
BIN_DIR="exec"
REMOVE=false
VERBOSE=false
...
if [ "${REMOVE}" = true ] && [ "${CLEAN}" = false ] ; then
  ...
  [[ -d "${HOME_DIR}/${BIN_DIR}" ]] && ( rm -rf ${HOME_DIR}/${BIN_DIR}  && printf '%s\n' "rm -rf ${HOME_DIR}/${BIN_DIR}" )
  ...
  [[ -d "${HOME_DIR}/manage_externals/manic" ]] && rm -f ${HOME_DIR}/manage_externals/manic/*.pyc  && printf '%s\n' "rm -f ${HOME_DIR}/manage_externals/manic/*.pyc"
  echo " "
fi
# Clean all the submodules if requested. NB: Need to check out them again before attempting subsequent builds, by sourcing ${HOME_DIR}/manage_externals/checkout_externals
if [ ${INCLUDE_SUB_MODULES} == true ]; then
  printf '%s\n' "Removing submodules ..."
  declare -a submodules='()'
  submodules=(${HOME_DIR}/sorc/*) 
# echo " submodules are: ${submodules[@]} (total of ${#submodules[@]}) " 
  ...
  printf '%s\n' "NB: Need to check out submodules again for any subsequent builds, " \
    " by sourcing ${HOME_DIR}/manage_externals/checkout_externals "
fi
#
echo " " 
